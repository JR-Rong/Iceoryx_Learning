...
char * node = NULL;
static int init_flag_ = 0;

/* Handle a SIGINT by cleaning up */
void deinit_top() {
    /* Fix tty */
    leave_cbreak_mode();
    exit(0);
...
{
#if 0
    //set time mic Effective
    struct _clockperiod new = {10000, 0};
    errno = 0;
    ClockPeriod(CLOCK_REALTIME,&new,NULL,0);
    printf("errno %d\n",errno);
#endif
    int i=0;
    number_of_CPUs = _syspage_ptr->num_cpu;
...
double get_cpu_load_mem(pid_t _pid) {
    if (!init_flag_) {
        init_top();
        init_flag_ = 1;
    }
    ...
    {   // update cpu telltales
        ...
        for ( i = 0; i < number_of_CPUs; ++i )
        {
            ...
            min_cpu_idle[i] = this_cpu_idle[i] < min_cpu_idle[i] ? this_cpu_idle[i] : min_cpu_idle[i];
            max_cpu_idle[i] = this_cpu_idle[i] > min_cpu_idle[i] ? this_cpu_idle[i] : min_cpu_idle[i];;
            tot_cpu_idle[i] += this_cpu_idle[i];
        }
    
        min_procs = this_procs < min_procs ? this_procs : min_procs;
        max_procs = this_procs > max_procs ? this_procs : max_procs;
        tot_procs+= this_procs;
    
        min_threads = this_threads < min_threads ? this_threads : min_threads;
        max_threads = this_threads > max_threads ? this_threads : max_threads;
    ...
    for (i = 0; i < active; i++) {
        ...
        if (_pid == te_p->status.pid) {
            // printf("te_p->time_used=%lld  -- now_p->total_time = %lld  --",te_p->time_used,now_p->total_time);
            ret_cpu_load += ((100.0 * te_p->time_used) / now_p->total_time);
            // printf("ret_cpu_load = %lf \n",(ret_cpu_load));
            ret_cpu_load += ((10000 * te_p->time_used) / now_p->total_time) % 100 / 100.0;
            // ret_cpu_load += (double)((te_p->time_used) / now_p->total_time);
        }
    }
...
//MemSize:单位:字节
int get_ProcMemsize(const int fd,unsigned int* MemSize)
{
    if (-1==fd)
    {
        return -1;
...
//获取进程占用内存
unsigned int get_proc_mem(pid_t _pid){
	
	char file_name[64]={0};
	int fd;
	sprintf(file_name,"/proc/%d/as",_pid);
    ...
    int ret = get_ProcMemsize(fd,&vmrss);
